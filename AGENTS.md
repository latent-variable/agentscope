...
- **Never work in a shared clone. Start code work in a dedicated worktree.** A clone you and several agents share is read-only territory: fine to read, inspect, `git log`. The moment you are going to WRITE, get your own worktree with `~/.agents/bin/worktree.sh <branch>`, **never bare `git worktree add`**. A plain worktree carries tracked files only; every gitignored build input (`.env*`, service-account JSON, API keys) stays behind and **nothing fails loudly** — the bundler inlines `undefined` and exits 0. Tear it down with `worktree.sh --remove`, never bare `git worktree remove`. Never assume the shared clone is on `main`, and never "fix" it by checking out under somebody else.
